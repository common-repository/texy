// add markitup compatible with wp
...
jQuery(document).ready(function() {
	...
	jQuery.fn.markItUp = function(settings, extraSettings) {
		...
		if (!options.root) {
			jQuery('script').each(function(a, tag) {
				miuScript = jQuery(tag).get(0).src.match(/(.*)jquery\.markitup(\.pack)?\.js$/);
				if (miuScript !== null) {
		...
		return this.each(function() {
			...
			function init() {
				...
				header = jQuery('<div class="markItUpHeader"></div>').insertBefore($$);
				jQuery(dropMenus(options.markupSet)).appendTo(header);

				// add the footer after the textarea
				footer = jQuery('<div class="markItUpFooter"></div>').insertAfter($$);
	...
	jQuery.fn.markItUpRemove = function() {
		return this.each(function() {
				$$ = jQuery(this).unbind().removeClass('markItUpEditor');
